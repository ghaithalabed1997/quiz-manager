...
class AdminController extends Controller
{
    ...
    public function add_new_question(Request $request)
    {
        if ($request->hasFile('audio')) {
            ...
            // Store file with a random hash name
            $path = $file->store('audios', 'public');

            // Get original file name (e.g., "test.mp3")
        ...
        if ($validator->fails()) {
            ...
            $q->audio_path = isset($path) ? $path : null;
            $q->audio_file = isset($originalName) ? $originalName : null;
            $q->status = 1;
